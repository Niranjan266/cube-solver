...
Optional: to send with a template stored in Resend, publish `backend/templates/support_email.html` as a Resend template (its variables are `{{{UPPERCASE}}}`, the syntax Resend uses) The server uses the template `support-message` by default (set `RESEND_TEMPLATE` to use another, or `none` for none). If sending with the template fails, the server sends the same design itself.
...
- `/robots.txt`, `/sitemap.xml` and `/llms.txt` are generated by the server. `/llms.txt` is a plain summary for AI assistants, and `frontend/llms.txt` is its source.
- `/guide` is served pre-rendered, so crawlers that do not run JavaScript still get the whole guide. After changing `frontend/guide.html` or `frontend/js/guide-data.js`, run `node tools/prerender-guide.mjs`.
- The vercel.app address redirects to https://cube.niranjand.in.

## The timer

`/timer` (linked from the top bar) is a speedcubing timer on its own page:

- **Press space (or tap) to start**; any key or tap stops it, and the next solve starts from the **Restart** button (so a stray press cannot start one). `Esc` cancels. Settings has **Hold to start** (hold until green, then let go) for the competition feel.
...
node tools/prerender-guide.mjs --check  # the pre-rendered guide matches guide.html + guide-data.js
